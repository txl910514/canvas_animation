...
(function (window) {
  ...
  u.storage.get = function (key) {
    ...
    if (ls) {
      ...
      if (!v) return;
      if (v.indexOf('obj-') === 0) return JSON.parse(v.slice(4));else if (v.indexOf('str-') === 0) return v.slice(4);
    }
  };
  /**
  ...
  u.isArray = function (source) {
    if (Array.isArray) return Array.isArray(source);else return source instanceof Array;
  };
  /**
  ...
  u.isEmpty = function (source) {
    ...
    if (u.isObject(source)) return JSON.stringify(source) === '{}';else return source.toString().length == 0;
  };
  /**
   * @description 判断元素的长度
   * @param {*} source 
  ...
  u.forEach = function (source, func) {
    ...
    for (var ikey in source) {
      var flag = func.apply(window, [_typeof(source) === "object" ? ikey : i++, source[ikey]]);
      if (flag == false) break;
    }
  };
  /******************************************** string 字符串 ***************************************************/
  ...
  */
  ...
  var arr3 = [2, ['w', 'q', 'r'], ['q', 'w']]; // var arr1 = ['w','q','r'];
  // var arr3 = ['w','q','r'];
  ...
  */
  ...
  u.array.indexOf = function (source, searchElement) {
    ...
    if (u.isArray(searchElement)) {
      u.forEach(source, function (i, item) {// if(item == searchElement){
        //     index = i;
        //     return false;
        // }
      });
  ...
  u.array.filter = function (source, filterProperty, getDeleteData) {
    ...
    u.forEach(source, function (i, item) {
      ...
      if (equal) ret.push(item);else retByDelete.push(item);
    });
  ...
  u.array.concat = function (source, array, keys) {
    ...
    if (keys && keys.length) {
      ...
      u.forEach(array, function (i, item) {
        ...
        var obj = u.getByObject(source, searchObj); // u.object.selectProperties
  ...
  u.array.sort = function (array, sort, order) {
    ...
    result.sort(function (a, b) {
      var aVal = a[sort];
      var bVal = b[sort];
      if (aVal > bVal) return order == "asc" ? 1 : -1;else if (aVal < bVal) return order == "asc" ? -1 : 1;
      return 0;
    });
    return result;
  };
  ...
  u.array.splice = function (array, item, newItem) {
    ...
    if (!u.array.isEmpty(array)) {
      ...
      if (index == -1) {
        ...
        if (newItem == undefined) array.splice(index, 1);else array.splice(index, 1, newItem);
      }
    }
